...
consumptionData = fraudulentData.groupby ( "invoice_date" ).agg (
    {"consommation_level_1": "sum", "consommation_level_2": "sum", "consommation_level_3": "sum",
    "consommation_level_4": "sum"} ).reset_index ()
consumptionData["invoice_date"] = pd.to_datetime ( consumptionData["invoice_date"] )
consumptionData.head ()


invoiceYears = consumptionData["invoice_date"].dt.year.unique()
...
def compareCustomers(year, counterType):
    ...
    fraudAvgConsumpLevel1 = fraudulentData[fraudulentData['client_id'] == fraudulentClient['client_id'].values[0]][
        'consommation_level_1'].mean ()
    nonFraudAvgConsumpLevel1 = \
    nonFraudulentData[nonFraudulentData['client_id'] == nonFraudulentClient['client_id'].values[0]][
        'consommation_level_1'].mean ()

    fraudDataFiltered = fraudulentData[
        (fraudulentData['counter_type'] == counterType) & (fraudulentData['invoice_date'] >= startDate) & (
                    fraudulentData['invoice_date'] <= endDate)]
    nonFraudDataFiltered = nonFraudulentData[
        (nonFraudulentData['counter_type'] == counterType) & (nonFraudulentData['invoice_date'] >= startDate) & (
                    nonFraudulentData['invoice_date'] <= endDate)]
    
    return fraudDataFiltered, nonFraudDataFiltered


fraudDataFiltered, nonFraudDataFiltered = compareCustomers(year=2019, counterType="ELEC")


# Calculate the average consumption level 1 for each month in the selected date range for each customer
fraudulentMonthly = fraudDataFiltered.groupby ( pd.Grouper ( key='invoice_date', freq='M' ) ).agg (
    {'consommation_level_1': 'mean'} ).reset_index ()
fraudulentMonthly['Monthly AVG'] = 'Fraudulent'
nonFraudulentMonthly = nonFraudDataFiltered.groupby ( pd.Grouper ( key='invoice_date', freq='M' ) ).agg (
    {'consommation_level_1': 'mean'} ).reset_index ()
