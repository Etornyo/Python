...
# Model training
lgb_model = LGBMRegressor(n_estimators=150, random_state=42)
lgb_model.fit(X_train, y_train)


# Model training (Example with XGBoost)
xgb_model = XGBRegressor(n_estimators=150, random_state=42)
...
test_predictions_lgb = lgb_model.predict(test_data[features])
test_predictions_xgb = xgb_model.predict(test_data[features])

# Ensemble the predictions using a simple average
ensemble_predictions = (test_predictions_lgb + test_predictions_xgb) / 2

# Ensemble predictions for the test dataset using simple average
ensemble_test_predictions = (test_predictions_lgb + test_predictions_xgb) / 2
